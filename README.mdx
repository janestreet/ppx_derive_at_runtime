...
be defined as `Sexp.t`. We open `Size.Export` so that `size_int` is in
...
derived value. For example, if there is no `size_float` in
...
For `[@@deriving size]`, all contexts other than type expressions
define attributes as `Nothing.t`. Other derivers may allow attributes
in these other positions, as needed. See where `Serializations` is
